...
#define L4_SpaceCtrl_kresources_accessible  (1 << 4)
#define L4_SpaceCtrl_prio(x)                ((x) << (L4_BITS_PER_WORD-8))
...
#define L4_ExReg_SrcThread(x)   (L4_ThreadNo(x) << 14)
...
#define L4_ErrInvalidClist      (12)
#define L4_ErrInvalidCap        (13)
...
/*
 * Capability IDs  (including ThreadIds)
 */
...
typedef union {
    L4_Word_t raw;
    struct {
        L4_BITFIELD2(L4_Word_t, index:L4_BITS_PER_WORD-4, type:4);
    } X;
} L4_CapId_t;

typedef L4_CapId_t L4_ThreadId_t;

#define L4_nilthread            L4_SpecialCapId(~3UL)
#define L4_anythread            L4_SpecialCapId(~0UL)
#define L4_waitnotify           L4_SpecialCapId(~1UL)
#define L4_myselfconst          L4_SpecialCapId(~2UL)

#define L4_rootserverno         0UL
#define L4_rootserver           L4_GlobalId(L4_rootserverno, 1)

L4_INLINE L4_CapId_t
L4_CapId(L4_Word_t type, L4_Word_t index)
{
    L4_CapId_t cap = { 0UL };
    cap.X.index = index;
    cap.X.type = type;
...
L4_SpecialCapId(L4_Word_t index)
{
    L4_ThreadId_t t = { 0UL };
    t.X.index = index;
    t.X.type = TYPE_SPECIAL;
...
L4_INLINE L4_Word_t
L4_CapIndex(L4_CapId_t c)
{
    return c.X.index;
}

L4_INLINE L4_Word_t
L4_CapType(L4_CapId_t c)
{
    return c.X.type;
}
...
L4_INLINE L4_Bool_t
L4_IsCapEqual(const L4_CapId_t l, const L4_CapId_t r)
{
    return l.raw == r.raw;
}

L4_INLINE L4_Bool_t
L4_IsCapNotEqual(const L4_CapId_t l, const L4_CapId_t r)
{
    return l.raw != r.raw;
}
...
/*
 * temporary - ResourceControl
 */
#define RC_CONTROL_MUTEX                0x2
#define RC_CONTROL_SPACE                0x3
#define RC_CONTROL_CLIST                0x4
#define RC_CONTROL_HEAP                 0x06
#define RC_CONTROL_SEGMENT              0x08
#define RC_CONTROL_NULL_SEGMENT_LIST    0x09
#define RC_CONTROL_SEGMENT_PERMS        0x0a
