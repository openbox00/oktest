...
map_fpage_fail:
    ...
    end_update ();
    this->unmap_fpage (dest_fp, false, kresource);
 ...
 * @returns_
 */
void generic_space_t::unmap_fpage(fpage_t fpage, bool unmap_all, kmem_resource_t *kresource)
{
    ...
    while (num)
    {
        ...
        if (EXPECT_FALSE(subtree))
        {
               ...
               )
            {
                ...
                pg->remove_subtree (this, size, false, kresource);
                flush = true;
                goto unmap_next_pgentry;
            }
            else
...
#ifdef CONFIG_WBTEST
int WBT_get_mapping(word_t* arg0, word_t* arg1, word_t* arg2)
{
    spaceid_t i_spaceid = spaceid(*arg0);
    word_t i_vaddr = *arg1;
    word_t i_seg = *arg2;
    word_t* o_offs = arg0;
    word_t* o_size = arg1;
    ...
    space_t *space;
    space = get_space_list()->lookup_space(i_spaceid);
    if (space == NULL) {
        return 0;
    }
    ...
    pgent_t::pgsize_e pgsize;
    bool exists;
    exists = space->lookup_mapping ((addr_t)i_vaddr, &pg, &pgsize);
    if (exists)
    {
        word_t rwx =
            (pg->is_readable(space, pgsize) << 2) |
            (pg->is_writable(space, pgsize) << 1) |
            (pg->is_executable(space, pgsize) << 0);
        *o_attr =
            ((word_t)(pg->get_attributes(space, pgsize)) << 3) | rwx;

        *o_offs = (word_t)pg->address(space, pgsize);
        if (get_current_space()->phys_segment_list) {
            phys_segment_t *segment = get_current_space()->phys_segment_list->lookup_segment(i_seg);
            if (segment) {
                *o_offs -= segment->get_base();
            }
        }
#ifdef CONFIG_PT_LEVEL_SKIP
        *o_size = hw_page_shift(pg->hw_pgsize(space, pgsize));
#else
        *o_size = page_shift(pgsize);
#endif
//        printf("space = %x, vaddr = %x exists=%d, phys=%x\n", i_spaceid.get_raw(), i_vaddr, exists, *o_phys);
