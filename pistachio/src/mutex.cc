 ...
 */
static mutex_t * mutex_create(mutexid_t mutex_id, kmem_resource_t *kresource);
...
SYS_MUTEX(mutexid_t mutex_id, word_t flags, word_t * state_p)
{
    ...
    mutex = get_current_space()->lookup_mutex(mutex_id);
...
SYS_MUTEX_CONTROL(mutexid_t mutex_id, word_t control)
{
    ...
    if (EXPECT_FALSE(!get_current_space()->can_access_kresources())) {
    ...
    if (control & MUTEX_CONTROL_DELETE) {
        ...
        /* XXX : SMT consideration */
        /* Delete the mutex. */
        mutex->mutex_lock.unlock();
        mutex_delete(mutex_id, mutex, kresource);
    }

    /* Do we need to create a new mutex? */
    if (control & MUTEX_CONTROL_CREATE) {

        /* Check caller has access to specified mutex */
        if (EXPECT_FALSE(!get_current_space()->mutex_range.is_valid(
            mutex_id.get_number()))) {
            current->set_error_code(EINVALID_MUTEX);
            goto error_out;
        }
        ...
        /* Create the mutex. */
        mutex = mutex_create(mutex_id, kresource);
...
static mutex_t *
mutex_create(mutexid_t mutex_id, kmem_resource_t *kresource)
{
    mutex_t * mutex = (mutex_t *) kresource->alloc(kmem_group_mutex, true);
...
static void
mutex_delete(mutexid_t mutex_id, mutex_t * mutex, kmem_resource_t *kresource)
{
    ASSERT(DEBUG, mutex);

    mutexid_table.remove(mutex_id);
    mutex->destroy();
    kresource->free(kmem_group_mutex, mutex);
}

/*
 *  Initialisation functions.
 */
void SECTION(SEC_INIT) init_mutex(word_t max_mutexes,
                                  kmem_resource_t *kresource)
{
    ...
    /* Create the mutexes. */
    void * new_table =
        kresource->alloc(kmem_group_mutexids, sizeof(mutex_t *) * max_mutexes, true);
